...
window.onload = function() {
  setup(); // Ensure setup is called to initialize the game
};
let resetGame = function() {
  console.log('resetGame called');
  // Reset gameplay elements and board state
  boardSquares.forEach(square => square.unsetPiece());
  setupPieces(); // Reinitialize pieces for a new game
  updateScoreDisplay(); // Make sure scores are updated

  // Reset other game state variables as needed, excluding scores
  turn = 1;
  currentPlayer = white; // Or whoever is decided to start the new game
  white.checked = black.checked = false;
  // etc.

  // UI updates as needed
  document.getElementById("turnInfo").textContent = "Player's turn: White";
};

let showEnd = function(message){
  console.log('showEnd called with message:', message);

  document.getElementById("endText").textContent = message;
  document.getElementById("endMessage").className = "overlay show";
  setTimeout(() => {
      document.getElementById("endMessage").className = "overlay"; // Hide the message overlay
...
function showPromotion(player) {
  // Get the promotion overlay and list elements
  var promotionOverlay = document.getElementById("promotionMessage");
  var promotionList = document.getElementById("promotionList");

  // Set the class of the promotion overlay to "show" to make it visible
  promotionOverlay.className = "overlay show";

  // Set the class of the promotion list to the player's color
  promotionList.className = player.color;

  var promotionOptions = promotionList.getElementsByTagName("li");
  for (var i = 0; i < promotionOptions.length; i++) {
    promotionOptions[i].addEventListener("click", function() {
      promote(this.dataset.piece);
      // Then close the promotion overlay
      closePromotion();
    });
  }
}
...
Piece.prototype.capture = function(){
  this.captured = true;
  let capturingPlayer = (this.color === "white" ? black : white); // Determine the player making the capture
  let pieceValue = pieceValues[this.type];
  if (pieceValue) { // If the piece has a value (i.e., is not a king)
      capturingPlayer.score += pieceValue; // Update the capturing player's score
      updateScoreDisplay(); // Call a function to update the score display on the webpage
  }
}

function updateScoreDisplay() {
  document.getElementById("whiteScore").textContent = "White Score: " + white.score;
  document.getElementById("blackScore").textContent = "Black Score: " + black.score;
}
